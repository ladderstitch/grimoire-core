...
1. **Map the portfolio and define optimal levels for each area.** List all current areas of investment: products, teams, initiatives, markets, asset classes. For each, define the optimal level — the investment level that maximizes return per unit of resource at current conditions. This may be: maintain share at current level (mature product), hit growth targets (scaling product), or minimum viable presence (exploratory bet). Without an explicit optimal level, surplus and deficit have no meaning.

2. **Identify areas running above optimal (surplus).** An area runs surplus when: marginal return on additional investment is declining; the area exceeds its target allocation; further investment produces maintenance of position rather than meaningful improvement; resources would produce higher returns if deployed elsewhere. Common surplus signals:
   - Revenue growing but at slower rate than investment
   - Market share at ceiling with further investment producing diminishing returns
   - Engineering team size exceeds what the product roadmap requires
   - Asset class exceeds target portfolio weight
   - Inventory above demand forecast

3. **Identify areas running below optimal (deficit).** An area runs deficit when: high-return opportunities exist that are resource-constrained; the area has the highest forward expected return per unit of investment but is under-resourced; it cannot execute its roadmap due to resource gaps. Common deficit signals:
   - Growth initiative blocked on headcount or capital
   - New market opportunity identified but unfunded
   - Asset class below target portfolio weight with strong forward return
   - Production line running below capacity with demand backlog

4. **Quantify the surplus and calculate the redirect.** For surplus areas: how much above optimal are they? What is the harvest rate — the rate at which resources can be extracted without degrading the area below its optimal level? Do not extract below optimal. For deficit areas: what level of resource would close the gap to optimal? Prioritize deficits by forward return per unit of resource.

5. **Design the reallocation mechanism.** Reallocation is not announcement — it requires a structural mechanism that enforces the redirect over time:
   - **Capital:** Modify budget allocation process to cap reinvestment in surplus areas and create explicit funding pools for deficit areas
   - **Headcount:** Establish hiring/transfer rules that gate headcount in surplus areas and open headcount in deficit areas
   - **Attention:** Restructure review cycles so surplus areas operate with less senior attention (maintenance mode), freeing leadership bandwidth for deficit areas
   - **Inventory/capacity:** Adjust ordering, production, or capacity planning to bring areas to optimal level

6. **Set the rebalancing trigger.** Define the conditions under which reallocation is reviewed and executed. Options:
   - **Calendar-based:** Quarterly or annual portfolio review compares all areas to optimal levels and triggers reallocation
   - **Threshold-based:** When an area exceeds its target by X%, reallocation is triggered automatically
   - **Event-based:** New market entry, competitive change, or product milestone triggers reassessment
...
7. **Execute and track reallocation effects.** Implement the redirect. Monitor both the surplus area (does it maintain optimal level after harvest?) and the deficit area (does the infusion improve its trajectory?). Reallocation that damages the surplus area without improving the deficit area indicates a wrong diagnosis — re-examine the optimal level definition.

## Rules

- **Never extract below optimal.** The goal is to harvest surplus, not damage the source. A cash cow that is over-harvested declines below its sustainable level. Define and respect the floor.
- **Deficit priority is forward return, not severity of deficit.** The largest deficit is not necessarily the highest priority for reallocation. Direct surplus to the deficit with highest expected return per unit of resource.
- **Reallocation requires a structural mechanism, not willpower.** Announcing that resources should shift is not reallocation. Build the budget, headcount, or process mechanism that enforces the redirect automatically. Human default is to re-accumulate into winners; the mechanism prevents that default.
- **The human default — pile more into winners — is a strong pull.** Stakeholders in surplus areas will resist harvest. Leaders of deficit areas may not advocate effectively for themselves. The rebalancing policy must be explicit, visible, and institutionally enforced to survive political pressure.
- **Apply at regular intervals, not once.** Portfolio areas drift continuously. A one-time rebalancing is not surplus reallocation — it is a restructuring. The principle applies as ongoing policy, revisited at the defined trigger interval.
...
- **Harvesting past optimal:** Taking too much from the surplus area, degrading it below its optimal level. The source area becomes a new deficit, and the reallocation produces net harm. Define the floor before harvesting.
- **Directing surplus to the wrong deficit:** Reallocating to the most visible deficit rather than the highest-return deficit. Political pressure, squeaky-wheel dynamics, and emotional attachment bias allocation toward visible distress rather than forward opportunity.
- **One-time reallocation without a recurring mechanism:** Executing a reallocation once and treating it as complete. Portfolio drift is continuous; rebalancing must be a repeating process, not a one-time restructuring.
- **Confusing reallocation with austerity:** Surplus reallocation is not budget cutting. It is optimization — maintaining surplus areas at their optimal level and directing the excess to higher-return uses. Framing it as "taking resources away" misses the point and creates unnecessary resistance.
- **Measuring surplus by absolute performance instead of relative to optimal:** A business unit growing 10% YoY may still be surplus if the optimal investment level is lower than current allocation. A unit declining 5% may be deficit if it has the highest forward return per unit of additional investment. Measure against the optimum, not the absolute performance level.
